...
type MarketMeta = {
  ...
  marketType?: string;
  bundleFormula?: string | null;
  bundleComponents?: Array<{ symbol: string; coef: number }> | null;
};
type MarketDef = {
  symbol: string;
  posLimit: number;
  tickSize: number;
  marketType: 'basic' | 'bundle';
...
export default function Home() {
  ...
  const [mkDefs, setMkDefs] = useState<MarketDef[]>([
    { symbol: "A", posLimit: 100, tickSize: 0.1, marketType: 'basic' },
  ]);
  ...
  const [resolutionData, setResolutionData] = useState<{
    ...
    leaderboard: Array<{
      ...
      finalPnL: number;
      positions: Record<string, { qty: number; value: number }>;
    }>;
  } | null>(null);
  ...
  const resolveGame = () => {
    if (!markets.length) return;
    const basicMarkets = markets.filter(mk => mk.marketType === 'basic' || !mk.marketType);
    if (!basicMarkets.length) return alert("No basic markets to resolve");

    const trueValues: Record<string, number> = {};
    for (const mk of basicMarkets) {
      const v = Number(prompt(`True value for ${mk.symbol}:`) || "");
      if (!(v > 0)) return alert(`Must provide positive true value for ${mk.symbol}`);
      trueValues[mk.symbol] = v;
    }
    socket?.emit("admin_resolve_game", { trueValues });
  };
      ...
      <main className={(theme === "dark" ? "bg-black text-white" : "bg-white text-zinc-900") + " min-h-screen p-6 space-y-6"}>
        ...
        <div className="space-y-3 max-w-xl">
          ...
          <div className="space-y-2">
            ...
            {mkDefs.map((m, i) => (
              <div key={i} className="space-y-2 border border-zinc-700 rounded p-2">
                <div className="flex gap-2 items-center">
                  ...
                  <select
                    ...
                    value={m.marketType}
                    onChange={(e) =>
                      setMkDefs((d) => d.map((x, idx) => (idx === i ? { ...x, marketType: e.target.value as 'basic' | 'bundle', bundleFormula: e.target.value === 'basic' ? undefined : (x.bundleFormula || '') } : x)))
                    }
                  >
                ...
                {m.marketType === 'bundle' && (
                  <input
                    ...
                    value={m.bundleFormula || ''}
                    onChange={(e) =>
                      setMkDefs((d) => d.map((x, idx) => (idx === i ? { ...x, bundleFormula: e.target.value } : x)))
                    }
                    placeholder="Formula (e.g., 2*A + B or A+B+C)"
                  />
                )}
              </div>
            ))}
            {mkDefs.length < 5 && (
              <div className="flex gap-2">
                <button
                  ...
                  onClick={() => setMkDefs((d) => [...d, { symbol: "X", posLimit: 100, tickSize: 0.1, marketType: 'basic' }])}
                >
                  + Add Basic Market
                </button>
                <button
                  className="bg-zinc-700 rounded px-3 py-1"
                  onClick={() => setMkDefs((d) => [...d, { symbol: "BUN", posLimit: 100, tickSize: 0.1, marketType: 'bundle', bundleFormula: '' }])}
                >
                  + Add Bundle Market
                </button>
              </div>
    ...
    <main className={(theme === "dark" ? "bg-black text-white" : "bg-white text-zinc-900") + " min-h-screen p-6 space-y-6"}>
      ...
      {isAdmin && !!markets.length && (
        <div className="border border-zinc-800 rounded-lg p-3 flex gap-3 flex-wrap">
          ...
          <button className="bg-purple-700 rounded px-3 py-1 font-semibold" onClick={resolveGame}>
            🏆 Resolve Game & Show Final Standings
          </button>
        </div>
      )}

      {/* Game Resolution Results */}
      {gameResolved && resolutionData && (
        <div className="border-4 border-purple-600 rounded-lg p-6 bg-purple-950 bg-opacity-30">
          <h2 className="text-3xl font-bold mb-4 text-center">🏆 Game Resolved! 🏆</h2>

          <div className="mb-6">
            <h3 className="text-xl font-semibold mb-2">True Values:</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {Object.entries(resolutionData.trueValues).map(([symbol, value]) => {
                const mk = markets.find(m => m.symbol === symbol);
                return (
                  <div key={symbol} className="bg-zinc-900 rounded p-3">
                    <div className="font-semibold">{symbol}</div>
                    <div className="text-2xl text-green-400">${value.toFixed(2)}</div>
                    {mk?.marketType === 'bundle' && mk.bundleFormula && (
                      <div className="text-xs text-zinc-400 mt-1">({mk.bundleFormula})</div>
                    )}
                  </div>
                );
              })}
          ...
          <div>
            <h3 className="text-xl font-semibold mb-3">Final Leaderboard:</h3>
              ...
              <table className="w-full text-sm">
                <thead className="bg-zinc-900">
                  <tr>
                    ...
                    <th className="text-right p-2">Position Value</th>
                    <th className="text-right p-2 font-bold">Final P&L</th>
                  </tr>
                </thead>
                <tbody>
                  {resolutionData.leaderboard.map((player, idx) => (
                    <tr key={player.userId} className={"border-b border-zinc-800 " + (idx === 0 ? "bg-yellow-900 bg-opacity-20" : idx === 1 ? "bg-zinc-700 bg-opacity-20" : idx === 2 ? "bg-orange-900 bg-opacity-20" : "")}>
                      <td className="p-2">
                        {idx === 0 ? "🥇" : idx === 1 ? "🥈" : idx === 2 ? "🥉" : idx + 1}
                      </td>
                      <td className="p-2 font-semibold">{player.name}</td>
                      <td className="text-right p-2">${player.cash.toFixed(2)}</td>
                      <td className="text-right p-2">${player.positionValue.toFixed(2)}</td>
                      <td className="text-right p-2 font-bold text-lg">
                        <span className={player.finalPnL >= 0 ? "text-green-400" : "text-red-400"}>
                          ${player.finalPnL.toFixed(2)}
                        </span>
                      </td>
                    </tr>
                  ))}
      ...
      <section className={`grid ${gridCols} gap-4`}>
        {markets.map((mk) => {
          ...
          return (
            <div key={mk.symbol} className={"border rounded-lg p-3 " + (mk.marketType === 'bundle' ? "border-purple-600" : "border-zinc-800")}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-3">
                    ...
                    {mk.marketType === 'bundle' && (
                      <span className="text-xs rounded bg-purple-700 px-2 py-0.5">BUNDLE</span>
                    )}
                  ...
                  {mk.marketType === 'bundle' && mk.bundleFormula && (
                    <div className="text-xs text-purple-400">Formula: {mk.bundleFormula}</div>
                  )}
                </div>
