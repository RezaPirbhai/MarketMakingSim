...
function marketMeta(mk) {
  return {
    ...
    marketType: mk.book.marketType,
    bundleFormula: mk.book.bundleFormula,
    bundleComponents: mk.book.bundleComponents,
  };
}
...
function ensureGameAdminCreate(code, defs) {
  ...
  defs.slice(0, MAX_MARKETS).forEach(d => {
    ...
    const marketType = d.marketType || 'basic';
    const bundleFormula = d.bundleFormula || null;

    // Validate bundle formula references existing markets
    if (marketType === 'bundle' && bundleFormula) {
      const testBook = new OrderBook({ symbol: 'TEST', marketType: 'bundle', bundleFormula });
      if (testBook.bundleComponents) {
        for (const { symbol: compSymbol } of testBook.bundleComponents) {
          if (!g.markets.has(compSymbol)) {
            console.warn(`Bundle ${symbol} references non-existent market ${compSymbol}`);
          }
        }
      }
    }
...
io.on("connection", (socket) => {
  ...
  // ADMIN: resolve game with true values
  socket.on("admin_resolve_game", ({ trueValues }) => {
    const g = games.get(socket.data.code); if (!g) return;
    if (g.roles.get(socket.id) !== "admin") return;

    try {
      // trueValues = { "A": 100, "B": 50, ... } for basic markets
      const allTrueValues = { ...trueValues };

      // Calculate bundle true values
      for (const [symbol, mk] of g.markets.entries()) {
        if (mk.book.marketType === 'bundle') {
          const bundleValue = mk.book.calculateBundleValue(trueValues);
          allTrueValues[symbol] = bundleValue;
        }
      }

      // Calculate final P&L for each player
      const playerPnL = new Map();
      const allUserIds = new Set();

      // Collect all user IDs
      for (const mk of g.markets.values()) {
        for (const userId of mk.book.positions.keys()) {
      ...
      for (const userId of allUserIds) {
        ...
        for (const [symbol, mk] of g.markets.entries()) {
          const pos = mk.book.getPosPublic(userId);
          totalCash += pos.cash;
          const trueValue = allTrueValues[symbol] || 0;
          totalPositionValue += pos.qty * trueValue;
          positions[symbol] = { qty: pos.qty, value: pos.qty * trueValue };
        }

        const finalPnL = totalCash + totalPositionValue;
        playerPnL.set(userId, {
          userId,
          name: g.usernames.get(userId) || 'Unknown',
          cash: totalCash,
      ...
      const leaderboard = [...playerPnL.values()].sort((a, b) => b.finalPnL - a.finalPnL);

      // Close all markets and set settlements
      for (const [symbol, mk] of g.markets.entries()) {
        mk.settlement = allTrueValues[symbol];
        mk.open = false;
      }

      // Broadcast resolution results
      io.to(room(socket.data.code)).emit("game_resolved", {
        trueValues: allTrueValues,
        leaderboard,
        message: "Game resolved! Final standings calculated.",
      });
