...
class OrderBook {
  constructor({ symbol, tickSize = 0.1, posLimit = 100, onTrade = null, marketType = 'basic', bundleFormula = null }) {
    this.symbol   = symbol;
    this.tickSize = tickSize;
    this.posLimit = posLimit;
    this.onTrade  = typeof onTrade === "function" ? onTrade : null;
    this.marketType = marketType; // 'basic' | 'bundle'
    this.bundleFormula = bundleFormula; // e.g., "2*A + B + 3*C"
    ...
    // Parse bundle formula if this is a bundle market
    if (this.marketType === 'bundle' && this.bundleFormula) {
      this.bundleComponents = this._parseFormula(this.bundleFormula);
    } else {
  ...
  _parseFormula(formula) {
    // Parse formulas like "2*A + B + 3*C" into [{symbol: 'A', coef: 2}, {symbol: 'B', coef: 1}, ...]
    if (!formula) return [];
    ...
    for (const part of parts) {
      ...
      // Match pattern: optional sign, optional coefficient, *, symbol
      // Examples: "+2*A", "A", "-3*B", "B"
      const match = part.match(/^([+-])?(\d+(?:\.\d+)?)?(\*)?([A-Z]+)$/i);

      if (match) {
        const sign = match[1] === '-' ? -1 : 1;
        const coef = match[2] ? parseFloat(match[2]) : 1;
        const symbol = match[4].toUpperCase();

        components.push({ symbol, coef: sign * coef });
      }
    }
  ...
  // Calculate true value for bundle given component true values
  calculateBundleValue(componentValues) {
    if (this.marketType !== 'bundle' || !this.bundleComponents) return null;

    let value = 0;
    for (const { symbol, coef } of this.bundleComponents) {
      if (componentValues[symbol] == null) {
        throw new Error(`Missing true value for component ${symbol}`);
      }
      value += coef * componentValues[symbol];
    }
    return value;
  }
